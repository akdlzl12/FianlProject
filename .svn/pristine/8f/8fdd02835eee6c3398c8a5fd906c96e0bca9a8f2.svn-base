...
<script>
     $(function() {
           ...
           console.log("초기화" + purchaseDateValue);
    		...
    		// 고객테이블 클릭 시 하이라이트 처리
   	       $("#btn-open-modal").click(function() {
   	    	   // 상세버튼을 눌렀을 때 화면에 나오는 조회 리스트를 클릭하지 않으면 리턴을 날려 정지시킴
   	    	   if (!$("tr.active").length) {
   	    		   return;
   	    	   }
   	    	  if(ridValue) {
	       ...
	       $("#mainTbody").on("click", ".detailhighlight", function() {
	            ...
	            console.log("초기화 후 값을 담음" + purchaseDateValue + "no" + no + "deleted" + deleted + "posValue" + posValue);
	         	...
	         	$.ajax({
	         		...
	         		success: function(result) {
	        			...
	        			$.each(result, function(index, item) {
	        				html += "<tr class='detailhighlight' rid='"+item.rid+"' id='"+(item.customer ? item.customer.id : '')+"' pos='"+item.pos.id+"' purchaseDate='"+item.purchaseDate+"' no='"+item.id+"' deleted='"+item.deleted+"'>"
